...
Процедура ОбновитьСписокРегламентныхЗаданий()
	...
	Для Каждого Регламентное из Регламентные Цикл
		...
		НоваяСтрока.Параметры.ЗагрузитьЗначения(Регламентное.Параметры);
...
Процедура ОбновитьСписокФоновыхЗаданий()
	...
	Для Каждого Фоновое из Фоновые Цикл
		...
		Если РегламентноеЗадание <> Неопределено Тогда
			...
			Если ЭлементыФормы.СписокФоновыхЗаданий.Колонки.Параметры.Видимость Тогда
				СписокПараметров = Новый СписокЗначений;
				СписокПараметров.ЗагрузитьЗначения(Фоновое.РегламентноеЗадание.Параметры);
				НоваяСтрока.Параметры = СписокПараметров;
			КонецЕсли;	
		КонецЕсли;
...
Процедура ПриОткрытии()
	Попытка
		...
		ОтборРегламентныхЗаданий = ВосстановитьЗначение("РегламентныеЗадания.Отбор");
...
Процедура СписокРегламентныхЗаданийПередНачаломДобавления(Элемент, Отказ, Копирование)
	...
	Если Диалог.ОткрытьМодально() = Истина Тогда
		...
		Строка.Идентификатор = РегламентноеЗадание.УникальныйИдентификатор;
		Если ЗначениеЗаполнено(РегламентноеЗадание.Параметры) Тогда
			Строка.Параметры.ЗагрузитьЗначения(РегламентноеЗадание.Параметры);
		КонецЕсли;	
...
Процедура СписокФоновыхЗаданийПриВыводеСтроки(Элемент, ОформлениеСтроки, ДанныеСтроки)
	Если ДанныеСтроки.Состояние = Строка(СостояниеФоновогоЗадания.Активно) Тогда 
		ОформлениеСтроки.Шрифт = Новый Шрифт(ШрифтыСтиля.ШрифтТекста,,, Истина);
	ИначеЕсли ДанныеСтроки.Состояние = Строка(СостояниеФоновогоЗадания.Завершено) Тогда
	ИначеЕсли ДанныеСтроки.Состояние = Строка(СостояниеФоновогоЗадания.ЗавершеноАварийно) Тогда	
		ОформлениеСтроки.ЦветТекста = Новый Цвет(255, 0, 0);
	ИначеЕсли ДанныеСтроки.Состояние = Строка(СостояниеФоновогоЗадания.Отменено) Тогда	
...
Процедура УстановитьОтборРегламентныхЗаданий(Кнопка)
	...
	Если Диалог.ОткрытьМодально() = Истина Тогда	
		...
		СохранитьЗначение("РегламентныеЗадания.Отбор", ОтборРегламентныхЗаданий);
