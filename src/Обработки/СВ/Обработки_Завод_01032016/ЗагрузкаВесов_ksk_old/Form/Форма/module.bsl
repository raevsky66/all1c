...
Процедура ОсновныеДействияФормыПечать(Кнопка)
	...
	ОбластьМакетаШапка.Параметры.Контрагент = _Документ.Контрагент;
	...
	Иначе
		Если  НовСтрока.Документ = Неопределено Тогда  
			...
			Если НовСтрока.ВходящийИсходящий = "Исходящий" Тогда  //сообщить("Заносим отгрузку");
				...
				Док.ДоговорКонтрагента = НовСтрока.Контрагент.ОсновнойДоговорКонтрагента;
				...
				СчетаУчета = БухгалтерскийУчетРасчетовСКонтрагентами.ПолучитьСчетаРасчетовСКонтрагентом(Фирма , НовСтрока.Контрагент, НовСтрока.Контрагент.ОсновнойДоговорКонтрагента);
				
				СтрДок = Док.ЭтотОбъект.Товары.Добавить();
				СтрДок.Номенклатура = НовСтрока.Номенклатура;
				СтрДок.Количество = НовСтрока.ВесГруженный - НовСтрока.ВесПустой;
				СтрДок.ВесПустой = НовСтрока.ВесПустой;
				СтрДок.ВесГруженный = НовСтрока.ВесГруженный;
				СтрДок.КоличествоПоДокументу = НовСтрока.ВесСВоздухом - НовСтрока.ВесПустой;
		...
		Если стр.Документ <> Неопределено Тогда 
			Для Каждого Стр2 из стр.Документ.Товары Цикл 
				Если стр.ВходящийИсходящий = "Исходящий" Тогда 
					стр.ВесСВоздухом = стр.ВесПустой+стр2.КоличествоПоДокументу;
...
Процедура ПриОткрытии()
	...
	//	    глВесы.ОткрытьВесы1(Сервер);				
	//	    глВесы.ОткрытьВесы2(Сервер);				
	//	    глВесы.ОткрытьВесы3(Сервер);				
	//	    глВесы.ОткрытьВесы4(Сервер);				
	//	    глВесы.ОткрытьВесы5(Сервер);				
	//		глВесы.ОткрытьВесы6(Сервер);				
	//		глВесы.ОткрытьВесы7(Сервер);				
	//		глВесы.ОткрытьВесы8(Сервер);				
	//		глВесы.ОткрытьВесы9(Сервер);		
			ОбновитьВес();
		Иначе
			глВесы = 0;
		КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РеализацияТоваровУслуг.ВесПустой + РеализацияТоваровУслуг.КоличествоПоДокументу КАК ВесСВоздухом,
	...
	Пока Результат.Следующий() Цикл 
		...
		//НовСтрока.ВесСВоздухомНетто = Результат.Количество;
		НовСтрока.ВходящийИсходящий = "Входящий";
	КонецЦикла;
...
Процедура ДоВзвеситьНажатие(Элемент)
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда 
		...
		_ДокОбъект.Товары[0].Количество = _ТекСтрока.ВесГруженный - _ТекСтрока.ВесПустой;
		_ДокОбъект.Товары[0].КоличествоПоДокументу = _ТекСтрока.ВесСВоздухом - _ТекСтрока.ВесПустой;
		Попытка 
			_ДокОбъект.Записать();
		Исключение
